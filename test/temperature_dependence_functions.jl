...
module TestGaussian
    ...
    A = [0 1 0 ; 0 0 0 ; 0 1 0]
    p = model_parameters(A)
    gaussianFunction = gaussian(@NT(shape = :U, norm_constant = 0.5, range = 20, T_opt = 295, β = -0.25))
    body_size_relative = p[:bodymass]
    testvalues = gaussianFunction(body_size_relative, 273.15, p)
    @test testvalues[1] ≈ 0.908 atol = 0.001
    @test testvalues[2] ≈ 0.908 atol = 0.001
    @test testvalues[3] ≈ 0.908 atol = 0.001
    @test length(testvalues) == size(A,1)

    body_size_relative = [10.0, 1.0, 10.0]
    testvalues = gaussianFunction(body_size_relative, 273.15, p)
    @test testvalues[1] ≈ 0.511 atol = 0.001
    @test testvalues[2] ≈ 0.908 atol = 0.001
    @test testvalues[3] == testvalues[1]
