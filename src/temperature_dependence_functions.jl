...
function extended_eppley(para_extended_eppley)
    ...
    m=para_extended_eppley[6]
    beta=para_extended_eppley[7]

    return m^beta*a.*exp.(b.*temp).*(1-((temp-z)./(w/2)).^2)
...
function quadratic(para_quadratic)
    ...
    q=para_quadratic[4]
    m=para_quadratic[5]
    beta=para_quadratic[6]
    k=8.617e-5
    return m^beta*c*exp(b*(-1/k*temp)+q*(-1/k*temp)^2)
...
function exponential_BA(para_exponential_BA)
    ...
    m=para_exponential_BA[4]
    beta=para_exponential_BA[5]

    k=8.617e-5 # Boltzmann constant

    return p0*m^beta*exp(-E/(k*temp))
...
function extended_BA(para_extended_BA)
    ...
    m=para_extended_BA[6]
    beta=para_extended_BA[7]
    k=8.617e-5 # Boltzmann constant
                                             |

    lT=1/(1+exp(-1/(k*temp)*(Ed-(Ed/topt+k*log(E/(Ed-E)))*temp)))
    return p0*m^beta*exp(-E/(k*temp))*lT
...
function gaussian(para_gaussian, Ushape=0)
    ...
    m=para_gaussian[5]
    beta=para_gaussian[6]

    if Ushape == 0
        return m^beta*p0*exp(-(temp-topt)^2/(2*s^2))
    elseif Ushape == 1
        return  m^beta*p0*exp((temp-topt)^2/(2*s^2))
    end
end
