...

{
    ...
    "category": "section",
    "text": "To describe a more classical unimodal relationship of biological rates with temperature, one can also use the extended Boltzmann Arrhenius function. This is an extension based on the Johnson and Lewin model to describe the decrease in biological rates at higher temperatures (and is still based on chemical reaction kinetics).$qi(T) = exp(q0) * M^\\beta_i * exp(\\frac{E}{kT * l(T)}) $Where l(T) is :$l(T) = \\frac{1}{1 + exp[\\frac{-1}{kT} + (\\frac{ED}{T{opt}} + k * ln(\\frac{E}{E_D - E}))]} $"
},
...

{
    ...
    "title": "Gaussian",
    "category": "section",
    "text": "A simple gaussian function (or inverted gaussian function depending on the rate) has also been used in studies to model the scaling of biological rates with temperature. This can be formalized by the following equation:$qi(T) = Mi^\\beta * q{opt} * exp[\\pm (\\frac{(T - T{opt})^2}{2s_q^2})] $"
},
